 ...
 * <p>
 * 命名规范
 * 【强制】命名规范：[前缀:]业务类型[:业务字段][:业务值]
 * 0）前缀： 可选。 用来区分不同项目，不同环境。
 * 3) 业务类型： 必填。 用来区分不同业务类型的数据缓存。 通常设置为表名。 同一key有多个业务类型时， 使用英文半角点号 (.) 分割，用来表示一个完整的语义。 如user.role 表示存储用户拥有的角色
 * 4) 业务字段: 可选。用来区分业务值是那个字段，通常设置为字段名。 同一key有多个业务类型时，业务字段对应多个业务类型。
