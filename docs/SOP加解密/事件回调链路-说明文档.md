...
| 字段 | 字段名称   | 接口回调（CALLBACK）     | 事件推送（EVENT_PUSH） |
| --- |--------|--------------------| --- |
| `type` | 消息类型   | `CALLBACK`         | `EVENT_PUSH` |
| `method` | 消息标识   | 接口名（如 `org.save`）  | 事件编码（如 `org.edit`、`user.add`） |
| `version` | 接口版本   | 有 `@Open(version=?)` | 无 |
| `callLogId` | 调用日志ID | 有（关联接口调用日志）        | 无 |
| `eventTriggerId` | 事件触发ID | 无                  | 有（关联事件触发日志，用于对账/幂等） |
| `bizContent` | 业务数据   | 业务方法组装的参数          | 事件触发时传入的事件内容 |
